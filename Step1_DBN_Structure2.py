...
def load_CodaLab_skel(ratio_train=0.9, ration_valid=0.1):
    ...
    f = file('Feature_all_train_realtime.pkl','rb' )
...
#     [train_set_feature_normalized, Mean1, Std1]  = preprocessing.scale(train_set_feature)
    scaler = preprocessing.StandardScaler().fit(train_set_feature)
    Mean1 = scaler.mean_
    Std1 = scaler.std_
    train_set_feature_normalized = normalize(train_set_feature,Mean1,Std1)
...
def test_GRBM_DBN(finetune_lr=0.1, pretraining_epochs=100,
...
#               n_outs=211, finetune_lr=finetune_lr)
#     dbn = GRBM_DBN(numpy_rng=numpy_rng, n_ins=528,
#         hidden_layers_sizes=[1000, 1000, 500],
#         n_outs=211)
    
#     dbn = GRBM_DBN(numpy_rng=numpy_rng, n_ins=528, hidden_layers_sizes=[1000, 1000, 500], n_outs=211)

#     dbn = GRBM_DBN(numpy_rng=numpy_rng, n_ins=528, hidden_layers_sizes=[2000, 2000, 1000], n_outs=211, finetune_lr=0.1)
#     dbn = GRBM_DBN(numpy_rng=numpy_rng, n_ins=528, hidden_layers_sizes=[2000, 2000, 1000], n_outs=211, finetune_lr=0.2)
    
#     model 1
 #   dbn = GRBM_DBN(numpy_rng=numpy_rng, n_ins=528, hidden_layers_sizes=[1000, 500], n_outs= 71, finetune_lr=0.1)
    
#     model 2
#    dbn = GRBM_DBN(numpy_rng=numpy_rng, n_ins=528, hidden_layers_sizes=[1000, 1000, 500], n_outs=71, finetune_lr=0.1)
    
#     model 3
    dbn = GRBM_DBN(numpy_rng=numpy_rng, n_ins=528, hidden_layers_sizes=[2000, 1000], n_outs=71, finetune_lr=0.1)
    ...
    filename = 'dbn_model1less_nonvar'+strftime("%Y-%m-%d-%H-%M-%S", gmtime())
    dbn.save(filename)
