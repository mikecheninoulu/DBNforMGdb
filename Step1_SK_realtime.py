...
Feature_all =  numpy.zeros(shape=(600000, (njoints*(njoints-1)/2 + njoints**2)*3),dtype=numpy.float32)
Targets = numpy.zeros( shape=(600000, STATE_NO*class_count+1), dtype=numpy.uint8)
    ...
    if file_count<650: 
        ...
        for gesture in gesturesList:
            ...
            if not valid_skel:
                ...
                #we delete gesutre those are longer than 300
                if endFrame < (startFrame + 150):                        
                    ### extract the features according to the CVPR2014 paper
                    Feature = Extract_feature_Realtime(Skeleton_matrix, njoints)
                    Target = numpy.zeros( shape=(Feature.shape[0], STATE_NO*class_count+1))
                    ...
                    for i in range(STATE_NO):  #HMM states force alignment
                        ...
                        targets = numpy.zeros( shape =(STATE_NO*class_count+1,1))
...
f = open('Feature_all_train_realtime.pkl','wb')
